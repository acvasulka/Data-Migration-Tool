...
export default function FMXPushModal({
  ...
  // Validation state
  const [validationResult, setValidationResult] = useState(null); // { validCount, invalidCount, rowErrors }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let idCache = {};
      try {
        const result = await buildIdCache(mappedRows, schemaType, url, em, pw);
      ...
      const payloads = mappedRows.map(row => {
        const p = transformRowToPayload(row, schemaType, idCache, cfIdMap, cfMeta);
      ...
      const result = { validCount, invalidCount, rowErrors };
      ...
      // Otherwise the validation UI will be shown for user decision
    })();
  }, [phase]);
  ...
  const createdIdsRef = useRef({}); // { rowIndex: createdId }
    ...
    (async () => {
      ...
      if (!payloads || payloads.length === 0) {
        ...
        payloads = mappedRows.map(row =>
          transformRowToPayload(row, schemaType, idCache, customFieldIdMap || {}, customFieldMetadata || [])
        );
      }

      // For update mode: build a name→id map of existing entities
      let existingEntityMap = {}; // { "Main Campus": 42, ... }
      if (pushMode === 'update') {
        setStatusMsg('Fetching existing records for update…');
        try {
          // Determine the name field for this entity type
      ...
      for (let i = 0; i < payloads.length; i++) {
        ...
        try {
          ...
          if (pushMode === 'update') {
            // Resolve existing entity ID by name/tag
          ...
          const res = await fetch('/api/fmx', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ siteUrl: url, email: em, password: pw, endpoint: reqEndpoint, method: httpMethod, payload: payloads[i] }),
          });
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button
              ...
              onClick={() => { cancelledRef.current = true; setValidationResult(null); setPhase('setup'); }}
            >
              Back
            </button>
            <button
              className="fmx-btn-primary"
              style={{ flex: 1, fontSize: 12, padding: '8px 0' }}
              onClick={() => { setSkipInvalid(true); setPhase('pushing'); }}
            >
